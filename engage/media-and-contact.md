---
layout: program-doc
title: "Media & Contact"
lane: engage
permalink: /engage/media-and-contact/
summary_short: "Typed contact routes for media, technical inquiry, institutions, and general correspondence."
summary_cards:
  - title: "Typed routing"
    body: "Please indicate your inquiry type so we can route your message to the right person."
  - title: "Response time"
    body: "We aim to respond within one week. Complex technical inquiries may take longer."
right_rail:
  related:
    - title: "Engage Overview"
      url: /engage/
    - title: "About the Research"
      url: /research-program/about/
    - title: "Publications"
      url: /publications/
  artifacts:
    - title: "Series Prospectus"
      url: /publications/white-papers/
    - title: "Subtitle Doctrine"
      url: /publications/white-papers/
  meta:
    type: "Contact"
    status: "Active"
    updated: "April 2026"
---

## Contact the Research Program

Please indicate your inquiry type so we can route your message appropriately.

### Media Inquiry

For journalists, editors, podcast hosts, and media professionals covering science, mathematics, publishing, or technology.

**What we can provide**: Series overview, author background, key claims with scope labels, falsification pack, publication timeline, citation data.

**Email**: [contact@panta-rhei.site](mailto:contact@panta-rhei.site) — subject line: "Media Inquiry"

### Technical Inquiry

For researchers, mathematicians, physicists, biologists, philosophers, and formal methods specialists with domain-specific questions about the framework, formalization, or specific claims.

**What we can provide**: Technical clarification, registry references, TauLib module pointers, guided tour recommendations, scope status for specific claims.

**Email**: [contact@panta-rhei.site](mailto:contact@panta-rhei.site) — subject line: "Technical Inquiry"

### Institutional Contact

For universities, research institutions, funding bodies, publishers, and organizations exploring collaboration, evaluation, or institutional engagement.

**What we can provide**: Prospectus, reviewer's dossier, seminar abstracts, formalization overview, publication metadata.

**Email**: [contact@panta-rhei.site](mailto:contact@panta-rhei.site) — subject line: "Institutional"

### General Contact

For all other correspondence — readers, supporters, general questions, suggestions, or feedback.

**Email**: [contact@panta-rhei.site](mailto:contact@panta-rhei.site)

---

## Additional Resources

- **[Series Prospectus]({{ '/publications/white-papers/' | relative_url }})** — 23-page reference document for academics and institutions
- **[Falsification Pack]({{ '/publications/white-papers/' | relative_url }})** — 220+ quantitative predictions with precision claims
- **[Reviewer's Dossier]({{ '/publications/white-papers/' | relative_url }})** — factual substrate for evaluators
- **[About the Research]({{ '/research-program/about/' | relative_url }})** — what the program is and how to engage

---

*We aim to respond within one week. Complex technical inquiries may take longer. We read every message.*
