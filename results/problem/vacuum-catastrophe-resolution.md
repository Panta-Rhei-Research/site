...
- **`Ω_Λ`** — the effective dark-energy density observable from the Hubble rate `H(z)`, CMB acoustic peaks, and BAO. **Distinct object** — a boundary-readout effect, not a bulk Lagrangian term. Computed in the framework as `Ω_Λ = κ_D(1 + ι_τ³) ≈ 0.6849` per *V.T234*, with the canonical-vs-tau-effective bridge characterised by *V.T69* ("dark energy as boundary readout artifact"). See [N20 falsification entry](/results/falsifications/n20-dark-energy-density-06849/) for the prediction and the comparison to Planck.
