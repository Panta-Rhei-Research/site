...
This is one of the most difficult and least routine claims in the whole project. [The books]({{ '/publications/research-monographs/' | relative_url }}) carry that burden in detail, and the key physical [results]({{ '/results/' | relative_url }}) are summarized on the site. The site can only make the shape of the bridge visible. But that visibility matters, because without it, the reader would not be able to distinguish the program from more ordinary attempts to map mathematics onto physics by selective interpretation and local fitting.
