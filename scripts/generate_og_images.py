...
def render_assets(records: list[dict[str, Any]], make_png: bool, make_webp: bool) -> None:
    ...
    # AUD-23 · Per-route custom-OG sentinel.
    # An SVG marked with this sentinel as its first non-XML-decl line is
    # treated as hand-authored and NEVER regenerated by this script. PNG
    # and WebP rasters are still rebuilt from the hand-authored SVG so the
    # production OG card stays in sync with whatever the SVG declares.
    #
    # Use case: the homepage OG card is a *data-carrying* card (master
    # constant + TauLib counts + τ-spectrum seal) rather than the templated
    # logo-plate. The hand-authored SVG is checked in at
    # assets/og/svg/index.svg and carries the sentinel comment below.
    CUSTOM_OG_SENTINEL = "@custom-data-carrying"

    for record in records:
        svg_path = SVG_DIR / f"{record['slug']}.svg"
        # Sentinel check: if the on-disk SVG has the custom-data-carrying
        # marker in its first ~3 lines, treat it as hand-authored and skip
        # the template-write. PNG + WebP still get rasterised below.
        is_custom = False
        if svg_path.exists():
            head = "\n".join(svg_path.read_text(encoding="utf-8").splitlines()[:3])
            is_custom = CUSTOM_OG_SENTINEL in head
        if not is_custom:
