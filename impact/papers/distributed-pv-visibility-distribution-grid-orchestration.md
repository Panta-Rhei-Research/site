...
**Security and OT integration barriers.** DOE's 2021 BTM-data projects highlight that utilities are reluctant to integrate BTM telemetry into control-room systems because of security, protocol mismatch, and legacy-system constraints.[^8] This is not irrational caution — distribution control systems are operational technology (OT) environments with strict cybersecurity requirements, and connecting a new physics-inference layer introduces potential security exposure. Any τ deployment must address OT security architecture from the beginning, not as an afterthought.
