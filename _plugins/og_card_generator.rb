...
# This builder is intentionally opt-in. Normal Jekyll builds use explicit
# page-level plate images or the site-wide fallback image so editorial loops stay
# fast; social-preview cards are generated by a separate review workflow.
#
# Enable with:
#   PANTA_RHEI_GENERATE_OG_CARDS=1 bundle exec jekyll build --baseurl ""
#
# Optional mode:
#   PANTA_RHEI_OG_CARD_MODE=svg  # SVG only, no PNG rasterization
#   PANTA_RHEI_OG_CARD_MODE=png  # SVG + PNG where rasterization is enabled
#
# Resolution chain (see _includes/head.html):
#   1. page.og_image (explicit override — Tier 2 hand-crafted images)
#   2. /assets/og-image.png          (site-wide fallback)
#   3. /assets/og-cards/<slug>.png   (only in explicit social-preview builds)
#   4. /assets/og-cards/<slug>.svg   (only in explicit social-preview builds)
#
...
module PantaRhei
  module OgCardGenerator
    ...
    ENABLE_ENV        = "PANTA_RHEI_GENERATE_OG_CARDS"
    LEGACY_ENABLE_ENV = "PANTA_RHEI_OG_CARDS"
    MODE_ENV          = "PANTA_RHEI_OG_CARD_MODE"
    ...
    def enabled?
      truthy_env?(ENABLE_ENV) || truthy_env?(LEGACY_ENABLE_ENV)
    end

    def mode
      raw = ENV.fetch(MODE_ENV, "png").to_s.strip.downcase
      %w[svg png].include?(raw) ? raw : "png"
    end
    ...
    def run(site)
      unless enabled?
        Jekyll.logger.info "OG cards:",
                           "disabled (set #{ENABLE_ENV}=1 to run social-preview builder)"
        return
      end
...
Jekyll::Hooks.register :site, :after_init do |site|
  site.config["generated_og_cards_enabled"] = PantaRhei::OgCardGenerator.enabled?
  site.config["generated_og_cards_mode"] = PantaRhei::OgCardGenerator.mode
end
